// Arrays de imagens para cada produto
let galleries = {
    1: {
        images: [
            "img/Bc2.jpg",
            "img/Bc3.jpg",
            "img/Bc4.jpg",
            "img/Bc5.jpg",
            "img/Bc6.jpg",
            "img/Bc7.jpg",
            "img/Bc88.jpg",
            "img/Bc9.jpg",
            "img/Bc10.jpg",
        ],
        currentIndex: 0
    },
    2: {
        images: [
            "img/K2.jpg",
            "img/K3.jpg",
            "img/K4.jpg",
            "img/K5.jpg",
            "img/K6.jpg",
            "img/K7.jpg",
            "img/K8.jpg",
        ],
        currentIndex: 0
    },
    3: {
      images: [
      "img/Q2.jpg",
       "img/Q3.jpg",
        "img/Q4.jpg",
         "img/Q5.jpg",
          "img/Q6.jpg",
           "img/Q7.jpg",
            "img/Q8.jpg",
        ],
      currentIndex: 0
    },
    4: {
      images: [
        "img/B2.jpg",
        "img/B3.jpg",
        "img/B4.jpg",
        "img/B5.jpg",
        
        ],
        currentIndex: 0
    },
    5: {
      images: [
        "img/N1.jpg",
        "img/N2.jpg",
        ],
        currentIndex: 0
    },
    6: {
      images: [
        "img/T2.jpg",
        "img/T3.jpg",
        "img/T4.jpg",
        "img/T5.jpg",
        "img/T6.jpg",
        ],
        currentIndex: 0
    },
    7: {
      images: [
        "img/Y2.jpg",
        "img/Y3.jpg",
        "img/Y4.jpg",
    "img/Y5.jpg",
    "img/Y6.jpg",
    "img/Y7.jpg",
    "img/Y8.jpg",
    "img/Y9.jpg",
        ],
        currentIndex: 0
    },
    8: {
      images: [
        "img/P2.jpg",
        "img/P1.jpg",
        ],
        currentIndex: 0
    },
    9: {
      images: [
       "img/V2.jpg",
       "img/V3.jpg",
        ],
        currentIndex: 0
    },
    10: {
      images: [
       "img/U2.jpg",
       "img/U3.jpg",
       "img/U4.jpg",
       "img/U5.jpg",
       "img/U6.jpg",
       "img/U7.jpg",
       "img/U8.jpg",
       "img/U9.jpg",
       "img/U10.jpg",
        ],
        currentIndex: 0
    },
  11: {
      images: [
       "img/R2.jpg",
       "img/R3.jpg",
       "img/R4.jpg",
        ],
        currentIndex: 0
    },
    12: {
      images: [
       "img/W2.jpg",
       "img/W3.jpg",
       "img/W4.jpg",
       "img/W5.jpg",
       "img/W6.jpg",
       "img/W7.jpg",
        ],
        currentIndex: 0
    },
    13: {
      images: [
       "img/M2.jpg",
       "img/M3.jpg",
       "img/M4.jpg",
       "img/M6.jpg",
       "img/M6.jpg",
       "img/M7.jpg",
       "img/M8.jpg",
       "img/M9.jpg",
        ],
        currentIndex: 0
    },
};

// Função para trocar de imagem
function changeImage(galleryId, direction) {
    let gallery = galleries[galleryId];
    gallery.currentIndex += direction;

    // Volta ao início ou ao fim
    if (gallery.currentIndex < 0) {
        gallery.currentIndex = gallery.images.length - 1;
    } else if (gallery.currentIndex >= gallery.images.length) {
        gallery.currentIndex = 0;
    }

    // Atualiza a imagem no HTML
    document.getElementById(`productImage${galleryId}`).src = gallery.images[gallery.currentIndex];
    
}

// Seleciona todos os itens do carrossel
const carouselItems = document.querySelectorAll('.carousel-item');
let currentIndex = 0;

// Função para mostrar o próximo texto
function showNextText() {
    // Remove a classe 'active' do item atual
    carouselItems[currentIndex].classList.remove('active');

    // Atualiza o índice para o próximo texto
    currentIndex = (currentIndex + 1) % carouselItems.length;

    // Adiciona a classe 'active' ao próximo item
    carouselItems[currentIndex].classList.add('active');
}

// Inicializa o carrossel
function initCarousel() {
    carouselItems[currentIndex].classList.add('active'); // Mostra o primeiro texto
    setInterval(showNextText, 3000); // Troca o texto a cada 3 segundos
}

// Chama a função para iniciar o carrossel
initCarousel();